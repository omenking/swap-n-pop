module.exports = function(game) {
   ...
   */
  function LoopSound(game, key, volume, start_time, end_time, test_end_time) {
    // need to use game.add.audio()... extending phaser.sound has issues when returning the extended object
    let phaser_sound = game.add.audio(key);

    phaser_sound.addMarker("start", 0, start_time, volume, true);
    phaser_sound.addMarker("main", start_time, end_time - start_time, volume, true);
    phaser_sound.addMarker("testLoopEnd", test_end_time, end_time - test_end_time, volume, true);

    phaser_sound.stay_muted = false;

    phaser_sound.onMarkerComplete.add(m => {
      console.log(m);
      if (m !== "main")
        phaser_sound.play("main");
    });

    phaser_sound.stop_loop = () => {
      phaser_sound.stop();
      phaser_sound.mute = true;
    }

    phaser_sound.play_loop = (key) => {
      if (phaser_sound.stay_muted !== true) {
        phaser_sound.mute = false;
        phaser_sound.play(key);
      }
    }
  ...
  return class Sounds {
    ...
    create() {
      ...
      this.msx_stage_critical = new LoopSound(game, "msx_stage_critical", this.msx_volume, 12.759, 26.85, 23);
    } 
