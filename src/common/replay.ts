...
export default class Replay {
  ...
  public static save(name,seed,inputs,callback) {
  ...
  public static load(name,callback){
    const dir      = store.get('replay_dir')
    const filename = path.join(dir,`${name}.replay`)
    var   inputs   = [[],[]]
    var   seed     = null
  ...
  public static random_seed (howMany=16, chars) {
